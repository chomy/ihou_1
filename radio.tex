...
振動を表す微分方程式である。$\kappa$を定数とすると、
\begin{equation}
\frac{d^2X}{dZ} + \kappa X = 0 \label{eq:wave_eq}
\end{equation}
の解は、
\begin{equation}
X=exp(\lambda Z) \label{eq:wave_eqX}
\end{equation}
と仮定すると、
\begin{eqnarray}
\frac{dX}{dZ} &=& \lambda e^{\lambda Z} \nonumber\\
\frac{d^2X}{dZ^2} = &=& \lambda^2 e^{\lambda Z}
\end{eqnarray}
これらを、微分方程式
